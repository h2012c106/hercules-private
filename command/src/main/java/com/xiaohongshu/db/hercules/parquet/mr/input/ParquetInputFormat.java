...
public class ParquetInputFormat extends HerculesInputFormat {
    ...
    protected List<InputSplit> innerGetSplits(JobContext context, int numSplits) throws IOException, InterruptedException {
        ...
        if (!options.getSourceOptions().getBoolean(ParquetInputOptionsConf.ORIGINAL_SPLIT, false)) {
    ...
    protected void initializeContext(GenericOptions sourceOptions) {
        schemaStyle = SchemaStyle.valueOfIgnoreCase(sourceOptions.getString(ParquetOptionsConf.SCHEMA_STYLE, null));
    ...
    private ParquetInputWrapperManager generateWrapperManager(GenericOptions options) {
        switch (schemaStyle) {
            ...
            case ORIGINAL:
                boolean emptyAsNull = options.getBoolean(ParquetInputOptionsConf.EMPTY_AS_NULL, false);
