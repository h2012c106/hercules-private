...
public class ParquetInputMRJobContext implements MRJobContext {
    ...
    public void configureJob(Job job, WrappingOptions options) {
        ExampleInputFormat.setTaskSideMetaData(job, options.getSourceOptions().getBoolean(ParquetInputOptionsConf.TASK_SIDE_METADATA, false));
        try {
            ExampleInputFormat.addInputPath(job, new Path(options.getSourceOptions().getString(ParquetOptionsConf.DIR, null)));
        } catch (IOException e) {
